...
public class NotificationsList {
    ...
    public void addNotification(int cid, int bid, long eid, String from, String message, String chan, String buffer_type, String message_type, String network, String avatar_url) {
        ...
        if (eid <= last_eid) {
            IRCCloudLog.Log("Refusing to add notification for seen eid: " + eid);
            return;
        }
    ...
    private void showOtherNotifications() {
        ...
        if (notifications.size() > 0 && notify) {
            for (Notification n : notifications) {
                if (!n.isShown()) {
                    IRCCloudLog.Log(Log.DEBUG, "IRCCloud", "Posting notification for type " + n.getMessage_type());
                    if (n.getMessage_type().equals("callerid")) {
                        ...
                        action = new NotificationCompat.Action(R.drawable.ic_wearable_add, "Accept", PendingIntent.getService(IRCCloudApplication.getInstance().getApplicationContext(), (int)(n.getEid() / 1000), i, (PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE)));
                        ...
                        i.putExtra("network", n.getNetwork());
                        i.putExtra("to", n.getNick());
                        action = new NotificationCompat.Action.Builder(R.drawable.ic_wearable_reply, "Message", PendingIntent.getService(IRCCloudApplication.getInstance().getApplicationContext(), (int)(n.getEid() / 1000), i, (PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE)))
                        ...
                        try {
                            ...
                            i.putExtra("eid", n.getEid());
                            action = new NotificationCompat.Action(R.drawable.ic_wearable_add, "Join", PendingIntent.getActivity(IRCCloudApplication.getInstance().getApplicationContext(), (int)(n.getEid() / 1000), i, (PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE)));
                        } catch (Exception e) {
                            action = null;
                        }
                    } else {
    ...
    private android.app.Notification buildNotification(String ticker, int cid, int bid, long[] eids, String title, String text, int count, Intent replyIntent, String network, ArrayList<Notification> messages, NotificationCompat.Action otherAction, Bitmap largeIcon, Bitmap wearBackground, HashMap<String, Bitmap> avatars) {
        ...
        builder.setContentIntent(PendingIntent.getActivity(IRCCloudApplication.getInstance().getApplicationContext(), 0, i, (PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE)));
            ...
            for (Notification n : notifications) {
                if (n.getBid() != lastbid) {
                    if (show) {
                        ...
                        if (!selfOnly) {
                            try {
                                IRCCloudLog.Log(Log.DEBUG, "IRCCloud", "Posting notification for type " + last.getMessage_type());
    ...
    public NotificationCompat.Builder alert(int bid, String title, String body) {
        IRCCloudLog.Log(Log.DEBUG, "IRCCloud", "Posting alert notification");
