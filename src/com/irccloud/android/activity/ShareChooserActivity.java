...
public class ShareChooserActivity extends FragmentActivity implements NetworkConnection.IRCEventHandler, BuffersListFragment.OnBufferSelectedListener {
    ...
    protected void onResume() {
        ...
        if (session.length() > 0) {
            ...
            if(getIntent() != null && getIntent().hasExtra(/*Intent.EXTRA_SHORTCUT_ID*/"android.intent.extra.shortcut.ID")) {
                onBufferSelected(Integer.valueOf(getIntent().getStringExtra(/*Intent.EXTRA_SHORTCUT_ID*/"android.intent.extra.shortcut.ID")));
            }
        } else {
